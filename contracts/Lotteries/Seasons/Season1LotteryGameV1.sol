...
import "../../Services/Interfaces/IRandomizer.sol";
...
contract Season1LotteryGameV1 is ALotteryGame, ReentrancyGuard {
    using LotteryDef for LotteryDef.Period;

    mapping(uint8 => uint8) public featuresByDay;
    bool[] public featuresByDayBool;
    ...
    function resetCycle() external override onlyAdmin onlyWhenCycleNotRunning {
        ...
        featuresByDayBool = new bool[](totalSteps);
    }
    ...
    function nextStep() public override nonReentrant {
        require(
            currentPeriod == LotteryDef.Period.GAME,
        ...
        IRandomizer(discoveryService.getRandomizerAddr()).getRandomNum();
    ...
    function setRandomDigit(uint256 randomNumber) external onlyWhitelisted {
        uint8 randomDigit;
        randomDigit = uint8((randomNumber % featuresByDay[currentStep]) + 1);

        winningCombination += randomDigit * (10 ** (currentStep + 1));
        featuresByDayBool[currentStep - 1] = true;
    }

    function emergencyGetRandomNumber() external onlyAdmin {
        //Use this require to avoid underflow
        require(
            currentStep > 1,
            "ERROR :: You can't use this function if the game is not started"
        );
        //If the previous features haven't been set, we call again Randomizer
        if (featuresByDayBool[currentStep - 2] == false) {
            IRandomizer(discoveryService.getRandomizerAddr()).getRandomNum();
        }
    }
